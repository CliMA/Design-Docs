...
\begin{empheq}[box=\eqnbox]{equation}\label{eq:Integral_Runoff_result}
    \overline{R}_l^{\rm sfc} =  \int_{x=0}^{\infty} dx \int_{i_c(x)}^{\infty} dP (P-i_c(x))f_P(P; \alpha_P, \beta_P)  f_x(x;\alpha_x, \beta_x).
\end{empheq}
...
SWE \equiv z_{\rm snow} \frac{\rho_{\rm snow}}{\rho_l}
\end{empheq}
...
%and the values of $\lambda s$ = 0.33 kJ/m/hr/K and $\rho_{\rm snow}$ = 200 kg/m$^3$ can be calibrated to fit the internal energy measurements. 
...
\begin{empheq}[box=\eqnbox]{equation}
   \tilde{I}_{\rm snow} =  \rho_{\rm snow}[c_{\rm snow}(T - T_0) - (1-l)  L_{f,0}].
\end{empheq}
...
\begin{empheq}[box=\eqnbox]{equation}\label{eq:i_integrated}
    z_{\rm snow}\frac{\partial \tilde{I}_{\rm snow}}{\partial t} = -[Q_{\rm surf}(T_{\rm surf}; \ldots) - Q_{\rm bottom}(T_{\rm bottom}; \ldots)] - S_R z_{\rm snow},
\end{empheq}
...
%Two options are given. As discussed in the section on snow water equivalent, we make several assumptions consistent with a bulk approach:
%\begin{framed}
...
%  where $K_s$ is soil thermal conductivity, calculated from soil physics module, in unit \hl{check with soil($J m^{-1} s^{-1} K^{-1}$)}; $T_{stw}$ is the temperature of snowpack (total standing water); $T_{topsoil}$ is temperature of topsoil, $z_{snow}$ and $z_{topsoil}$ is the thickness of snowpack and topsoil layer 
%\hl{why not use soil thermal conductivity directly? we calculate it in the soil model.}\\
%\hl{sure, if soil model has it, no reason to calculate again, commented out the next section for soil thermal conductivity}\\
%\hl{the conductivity of soil is ~3-30x the conductivity of snow, so we might also be able to approximate it using just $K_soil$}
...
% \begin{equation*}\label{e:root_soil_water_conservation}
% \begin{aligned}
% \frac{\partial  \vartheta_{base}}{\partial t} =  \sum_i (K_{root,i} (z_{soil,i} + \psi_{soil,i} + P_{soil,i} - (z_{base} + \psi_{base} + P_{base}))\\ - \sum_i (K_{leaf} (z_{base} + \psi_{base} + P_{base} - (z_{leaf,j} + \psi_{leaf,j} + P_{leaf,j}))
% \end{aligned}
% \end{equation*}

% \begin{equation*}
%     \begin{aligned}
%     \frac{\partial  \vartheta_{leaf,j}}{\partial t} = (K_{leaf,j} (z_{base} + \psi_{base} + P_{base} - (z_{leaf,j} + \psi_{leaf,j} + P_{leaf,j})) - S)
%     \end{aligned}
% \end{equation*}
\newpage
...
%b is psi 50, alpha/beta is power thing (c)

where \\

                                                %   # -Scholz et al. 2011 Book, Hydraulic Capacitance: Biophysics and Functional Significance,
                                                %   C can also be related to P50 see same book
\begin{itemize}
    %\item $h = \psi + z$
    %\item $C_{base}$ = (79 + 8*height)*1e-6 is the tree capacitance [kg m−3 Pa−1] (from Pierre, Scholz book)
    %\item $C_{leaf}$ =  [kg m−3 Pa−1] (from Pierre, Scholz book)
    \item n = number of roots
    %\item b = Weibull scale parameter % for vulnerability curve from root tip to base
    \item $\alpha$ = Weibull shape parameter (root to base/underground) %for vulnerability curve from root tip to base
...
\begin{tabular}{llllll}
\hline
Parameter   & Description     & Unit & Source \\ \hline
%$C$ & blah &  ? & FETCH?\\
$\psi_{50}$ & blah &  ? & sperry? fetch? 2015 paper? \\ 
$k_{max}$ & blah  & ? &  ? \\ [2ex]
other variables & Description  & Units & Definition \\ \hline
blah  & blah &  blah & ?       \\
