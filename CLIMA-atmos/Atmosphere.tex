...
\begin{multline}
 ...
 = -\divergence (\rho \vec{F}_R) - \divergence \bigl(\rho (\vec{J} + \vec{D})\bigr) + \rho Q  \\
  +\divergence \left(\rho W_c \vec{k} \right)  - \divergence (\vec{u} \cdot \rho\vec{\tau)} %+ \rho \vec{u} \cdot \vec{F}_{\vec{u}} \\
...
The Vreman \citep{vreman2004} eddy-viscosity model characterised by low dissipation near wall-boundaries and transitional flow regimes. The eddy viscosity for this model is given by 
\begin{equation}\label{eq:vreman-sgs}
\nu_{e} = 2.5 C_{s}^2 \sqrt{\frac{B_{\beta}}{\alpha_{ij}\alpha_{ij}}} 
\end{equation}
where
\begin{align}
...
i,j = (1,2,3)
\end{align}
$C_s$ is the constant Smagorinsky coefficient and $u_{i}$ represent the components of the resolved scale velocity vector. $\Delta_{m}$ is the mixing length, determined in a manner consistent with that of the anisotropic Smagorinsky-Lilly model. Corrections for stratified flows are also applied as stated in equation  \ref{eq:Lilly_strat}. Eddy-diffusivity is then given $D_{e} = \frac{\nu_{e}}{Pr_{t}}$. No explicit filtering is prescribed in this model.
...
\end{equation}
so that when the divergence of the momentum equation is taken, a divergence equation of the form \eqref{e:divergence} results \citep[cf.][]{Jablonowksi11a}. 

In GCM configurations with much larger horizontal than vertical mesh scales, vertical diffusion may impose severe time step restrictions. Therefore, an anisotropic viscosity for divergence damping is desirable. NICAM uses no divergence damping in the vertical ($\vec{\nu}_D \cdot \vec{k}=0$) and $\nu_{D, h} \approx 0.1 c_{s, 0}^2 \Delta t_s$ for the horizontal viscosity components at $O(100~\mathrm{km})$ horizontal resolution \citep{tomita:2004}. Here, $c_{s,0}$ is a reference speed of sound and $\Delta t_s$ is the explicit substep in time for horizontal sound waves in their horizontally explicit and vertically implicit (HEVI) time stepping strategy, with substepping for horizontal sound and gravity waves. In LES configurations, an isotropic viscosity for divergence damping may be used.
...
\begin{enumerate}
    ...
    \] 
    where $\vec{n}$ is the \emph{inward} pointing unit normal vector at the boundary. This generally arises from a flux boundary condition on a diffusive flux (such as the moisture flux $-\vec{\mathcal{D}}_t \circ \grad q_t$) and is implemented as a condition on the flux (rather than a condition on the gradient itself).
\end{enumerate}
...
\[
E = \vec{n} \cdot (\rho \vec{d}_{q_{v}})\bigl|_\mathrm{sfc} = -\vec{n} \cdot (\rho \vec{\mathcal{D}}_t \circ \grad q_t)\bigl|_\mathrm{sfc}.
\]
...
\[
\left. \rho W_c\right|_\mathrm{sfc} = \rho W_c (\vec{k} \cdot \vec{n}).
\]
...
\begin{itemize}
    ...
    \item \textbf{Re-entrant channel}: Impenetrable, vapor impermeable, insulating, free-slip walls on two opposing sides (side walls of channels), and periodic boundary conditions at the other two sides, with outflow out of one side being equal to inflow through the opposite side. \fxg{Why is this called re-entrant but the inflow normal velocity is zero? TS: The inflow normal is not zero on the two opposing sides that are re-entrant; the boundary conditions at those two sides are periodic. So the topology is a torus.}
...
\end{equation}
for a sponge layer at the top of the domain at altitude $z_T$. The parameters in this formulation are the altitude $z_D$ where the sponge starts and the maximum relaxation coefficient $\alpha_{\max}$. Empirically, maximum relaxation coefficients around $\alpha_{\max} = (20 \Delta t)^{-1}$, with the model time step $\Delta t$, have been found to work well \citep{Jablonowksi11a}.
...
\chapter{Reference State and Initial Conditions}\label{c:reference_state}
...
\begin{equation}
    \vec{n} \cdot \rho (\vec{J} + \vec{D})_\mathrm{sfc} = \mathrm{MSEF}(\vec{x}, t).
\end{equation}
To mimic the structure of the thermal bubble specified through the initial condition, we use the moist static energy flux
\begin{equation}
     \mathrm{MSEF}(\vec{x}, t) =  F_0(t) \times \left\{ \begin{array}{ll}
 1 & \text{if } r \leq a\\
 e^{-(x - a)^2/\sigma^2} & \text{if } r > a,\\
\end{array} \right.
\end{equation}
...
\begin{equation}
 ...
 \end{array}\right.
\end{equation}
where $t_0=0$ and $t_1 = 500~\mathrm{s}$ (that is, the energy flux at the surface has a top-hat profile in time). \hl{[You may want to experiment with the amplitude of the flux and the duration for which it's on, to see when you get a reasonable bubble.]}
...
\caption{2D density current stabilized via a constant coefficient Smagorinsky-Lilly SGS. Potential temperature $\theta$ at $t=900\,\mathrm{s}$ (top) and at $t=1200\,\mathrm{s}$ (bottom). Grid resolution: $\Delta x = \Delta z = 25\,\mathrm{m}$. \fxg{I assume this is the 900 second plot - there is no other plot. Which polynomial order?}
}
