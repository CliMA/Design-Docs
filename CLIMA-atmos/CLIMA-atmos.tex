...
\end{equation}
[\fxg{What happened to $\Tvector^N_{I}(\vec{Y})$? I've added back in.  We can use this form whereby the total variable is used but if we use an iterative solver then it behooves us to use warm restarts, otherwise the iterative solver has to work harder to get the solution to get first to the reference state and then to the perturbation from the reference state. This situation is easy to see for cases when the solution is not far away from the reference field.  If not the case, then is not too big of an issue.}]
...
[\fxg{Would be good to know if we plan to use such states. If not then the 1D-IMEX problem becomes very simple for us and so we can optimize based on this since only one linear system has to be solved for all columns but with different RHS.}]
...
To discretize the equations in time using a partitioned Runge-Kutta method, let us describe the method using the following abstract 3-partition:
\[
\diff{\vec{Y}}{t}= F_1 + F_2 + F_3. 
\]
where the wave speed $\mathcal{S}$ of the different components are ordered as follows $\mathcal{S}(F_3) > \mathcal{S}(F_2) > \mathcal{S}(F_1)$.
The algorithm describing the 3-partition RK method is highlighted in Alg.\ \ref{alg:3-prk}.
\begin{algorithm}
\label{alg:3-prk}
\begin{algorithmic}
\State
\Function{3-Partition RK}{}
\State $\vec{Y}^{(i)}=\vec{Y}^n$ 
\For{$i=1:I_{stages}$} 
\State $\vec{Y}^{(j)}=\vec{Y}^{(i)}$ \Comment update $F_1(\red{ \vec{Y}^{(i)} })$
\For{$j=1:J_{stages}$} 
\State $\vec{Y}^{(k)}=\vec{Y}^{(j)}$ \Comment update $F_2(\blue{ \vec{Y}^{(j)} })$
\For{$k=1:K_{stages}$} 
\State $\vec{Y}^{(k)}=\vec{Y}^{(j)} + \Delta \tau \left[ F_1(\red{ \vec{Y}^{(i)} }) + F_2(\blue{ \vec{Y}^{(j)} }) + F_3(\vec{Y}^{(k)}) \right]$
\EndFor %k
...
\end{algorithmic}
\end{algorithm}
In Alg.\ \ref{alg:3-prk} the red and blue font indicates that those terms are frozen within the $k$-loop and is what allows a performance gain. Also, 
$\Delta \tau=\Delta t a_i^I a_j^J a_k^K$ where $\Delta t$ is the time-step of the slowest modes ($F_1$), the RK coefficients are written in low-storage form, and the order of each of the three partitions can be of different order. If all three partitions are of the same order then effective time-step of each partition is 
\[
...
If the embedded partition described in Alg.\ \ref{alg:3-prk} yields an insufficiently small time-step to maintain stability of the fastest waves then substepping can be added in a time-splitting approach as shown in Alg.\ \ref{alg:3-prk_v2} where an additional loop ($m$-loop) is added before the $k$-loop.
\begin{algorithm}
\label{alg:3-prk_v2}
\begin{algorithmic}
\State
\Function{3-Partition RK with Substepping}{}
\State $\vec{Y}^{(i)}=\vec{Y}^n$ 
\For{$i=1:I_{stages}$} 
\State $\vec{Y}^{(j)}=\vec{Y}^{(i)}$ \Comment update $F_1(\red{ \vec{Y}^{(i)} })$
\For{$j=1:J_{stages}$} 
\State $\vec{Y}^{(m)}=\vec{Y}^{(j)}$ \Comment update $F_2(\blue{ \vec{Y}^{(j)} })$
\For{$m=1:M_{substeps}$} 
\State $\vec{Y}^{(k)}=\vec{Y}^{(m)}$
\For{$k=1:K_{stages}$} 
\State $\vec{Y}^{(k)}=\vec{Y}^{(m)} + \Delta \tau \left[ F_1(\red{ \vec{Y}^{(i)} }) + F_2(\blue{ \vec{Y}^{(j)} }) + F_3(\vec{Y}^{(k)}) \right]$
\EndFor %k
...
\end{algorithmic}
\end{algorithm}
In Alg.\ \ref{alg:3-prk_v2}
$\Delta \tau=\frac{\Delta t}{M_{substeps}} a_i^I a_j^J a_k^K$.
