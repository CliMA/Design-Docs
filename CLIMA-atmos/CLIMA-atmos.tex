...
\hl{\textbf{Frank}: Is this what you actually implemented? Or did you use a different reference state. Note also that I changed to subscript $r$ for the reference state, to avoid confusion with thermodynamic reference state variables.}
[\fxg{Tapio, it's up to the user what they want to use for the reference. In the Balance Law solver we introduced the reference state via the AUX variables.  We then perform the linearization based on these AUX variables.}]

Solving for the fastest waves implicitly by a linear solve requires linearization of the fastest tendency terms. A simple and commonly used choice of reference state for the linearization is a dry isothermal atmosphere at rest in hydrostatic equilibrium. Given the reference state temperature $T_{r}$, the hydrostatically balanced pressure is
\begin{equation}
    p_r = p_{\mathrm{MSLP}} \exp (-z/H),
\end{equation}
where $p_{\mathrm{MSLP}}$ is the mean sea level pressure, and 
\begin{equation}
    H = \frac{R_d T_{r}}{g}
\end{equation}
is the scale height corresponding to the constant temperature $T_{r}$. The density then follows from the ideal gas law as
\begin{equation}
    \rho_r = \frac{p_r}{R_d T_{r}} = \frac{p_{\mathrm{MSLP}}}{R_d T_{r}} \exp (-z/H).
\end{equation}
The reference state is assumed at rest, $\vec{u}_r = 0$, consistent with geostrophic and hydrostatic balance in this isothermal atmosphere. Because the reference state is assumed dry, so $q_k= 0$ for $k \in \{ t, v, l, i, p\}$ in the reference state.
...
\end{equation}
And the linear component of the source term \hl{[So you are including external gravity waves and inertia-gravity waves in the implicit part. Do we really want to do that? It seems unusual for LES, and probably gives rise to dispersive errors for Rossby waves in GCMs.]}
[\fxg{The way this section was initially written represented what we COULD do.  I am now modifying this section to reflect what is actually done in the code.}]
is 
\begin{equation}
...
\right)
\end{equation}
which is linear with respect to $\rho'$. To obtain a system that is linear in state variables, we also need to express the perturbation  pressure $p'= p - p_r$ as a linear function of state variables, which we can obtain from the ideal gas law (neglecting moisture perturbations affecting the gas constant)
\begin{equation}\label{eq:pressure_perturb}
p' \approx R_d (\rho T)' 
\end{equation}
once we have $(\rho T)'$. The latter we get from the internal energy perturbation per unit volume,
\begin{equation}
...
\end{equation}
Combining \eqref{eq:pressure_perturb}--\eqref{eq:temperature_perturb} gives the perturbation pressure as a linear function of state variables. 
...
\hl{@TS: please check all this. All we are given from Stevens are $q_{tot}, \theta_{liq}$, and $q_{liq}$; very important, however, is that Stevens says the following which we currently not considering in the saturation adjustment functions for this test case:

``Modeling groups were also asked to \emph{standardize their thermodynamic calculations} so that the initial state corresponded to a cloud layer between 600 and 800 m with $q_{liq}$ =0.45 g/kg at cloud top.''}
...
\theta_l(z) = 
    \begin{cases}
    289.0\;{\rm K} & z\leq z_i,\\
    297.5 + (z - z_i)^{1/3}\;{\rm K}& z > z_i,
    \end{cases}
\end{equation}
...
%SM: corrected pressure and density calculation that returns the density as expected by Stevens: \rho(@0 m)=1.22 kg/m3  and \rho(@zi)=1.13 kg/m3:
%
...
\]
thus completely specifiying the initial state. 

%
%\hl{Simone: Try the above. It's easy and consistent with anelastic approximation.}
%
%We can do so by first computing a pressure associated with a neutral dry atmosphere at $\theta(z)= \theta_l(z=0)=289\,{\rm K}$ according to:
%\begin{equation}
%p_d(z) = p_0\left(1 - \frac{g\,z}{c_{pd}\,\theta_{l,z=0}} \right) 
%\end{equation}.
%
%As per \citet{Stevens05a}, the following thermodynamic constants are used to initialize this state: $c_p = c_{p d}= 1 .015\,\mathrm{kJ\,kg^{-1}\,K^{-1}}$, $L_{v,0}=2.47\,\mathrm{MJ\,kg^{-1}}$, $R_d=0.287\,\mathrm{kJ\,kg^{-1}\,K^{-1}}$.
%
%By means of the equation of state for moist atmospheres, initial density is then found as
%\begin{equation}
%\rho(z) = \frac{p_0}{R_m\theta_l(z)}\left(\frac{p}{p_0})^{c_{pm}/R_m - 1}
%\end{equation}
%where 
%\[
%c_{pm} = c_{p d} + (c_{p v} - c_{p d})q_t + (c_{p l} - c_{p v})q_l\]
%and 
%\[R_m=R_d\left[1 + (\eps_{dv}-1)q_t - \eps_{dv} q_l\right]
%\]
%with moist constants $c_{p d} = 1 .015\,{\rm kJ\,kg^{-1}\,K^{-1}}$,  $c_{p v} = 1.859\,{\rm kJ\,kg^{-1}\,K^{-1}}$, $c_{p l} = 4.181\,{\rm kJ\,kg^{-1}\,K^{-1}}$, $p_0=1.0178e5\,{\rm Pa}$.
%The calculated surface air density measures $\rho_0=1.22\,{kg\,m^{-3}}$ and measures $\rho_{zi}=1.13\,{kg\,m^{-3}}$ just below the cloud top as reported in \cite{Stevens05a}.
%%%%
%Other quantities that may be worth calculating are the virtual absolute and potential temperatures. 
%
%\[
%T(z) = \theta(z) - \frac{gz}{c_p}.
%\]
%Eq. (\ref{eq:betts1973}) is non-linear so that the solution is found via a root finding algorithm.
%
The virtual temperature and potential temperature are given by equations (\ref{e:virtual_temp,e:virtual_pottemp}) rewritten below in absence of ice as
\begin{equation}
T_v = \left[1 + (\eps_{dv}-1)q_t - \eps_{dv} q_l\right] T,
\end{equation}
\begin{equation}
\theta_v = \left[1 + (\eps_{dv}-1)q_t - \eps_{dv} q_l\right] \theta    
\end{equation}
where $\eps_{dv}\approx 1.61$. The initial state of all the quantities described above are plotted in Figure \ref{dycomsInitFig}
