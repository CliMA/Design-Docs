...
\hl{TODO: update SGS fluxes of moisture and energy in code according to this.}
...
\]
is the sum of kinetic energy, enthalpy $I + R_m T$, and potential energy $\Phi$.\footnote{The sum of enthalpy and potential energy is commonly referred to as the moist static energy. It is materially conserved in adiabatic, hydrostatic displacements.} We model the unresolved specific energy flux diffusively, so that the SGS flux of total energy becomes
\begin{equation}\label{e:SGS_energy_flux}
\vec{J} + \vec{D} = - \vec{\mathcal{D}}_t \circ \nabla (e^{\mathrm{tot}} + R_m T).
\end{equation}

While not needed for implementation, it facilitates the interpretation of this total energy flux and the formulation of boundary conditions to consider the two components $\vec{J}$ and $\vec{D}$ separately. We can model the SGS fluxes of water vapor ($\vec{d}_{q_v}$), liquid water ($\vec{d}_{q_l}$), and ice ($\vec{d}_{q_i}$) separately as diffusive fluxes, 
\begin{equation}\label{e:water_diffusion}
    \vec{d}_{q_k} = - \vec{\mathcal{D}}_t \circ \nabla q_k,
\end{equation}
where $k \in \{v, l, i\}$. With the same diffusivity $\vec{\mathcal{D}}_t$ for all phases, this ensures that the SGS flux of total specific humidity, $\vec{d}_{q_t} = \vec{d}_{q_v} + \vec{d}_{q_l} + \vec{d}_{q_i}$, satisfies the closure \eqref{eq:sgs-shum-flux}. The energy flux associated with the SGS flux of water then is $\vec{D} = (e_v^{\mathrm{tot}} + R_v T) \vec{d}_{q_v} + e_l^{\mathrm{tot}} \vec{d}_{q_l} +  e_i^{\mathrm{tot}} \vec{d}_{q_i}$,
and the SGS sensible heat flux can be calculated as the residual
\[
    \vec{J} = - \vec{\mathcal{D}}_t \circ \nabla (e^{\mathrm{tot}} + R_m T) - \vec{D}.
\]
Using the definition of total energy as the weighted sum of the constituent energies \eqref{e:energy_sum} and the chain rule of differentiation yields the gradient
\begin{multline*}
\nabla (e^{\mathrm{tot}} + R_m T) \\
...
\end{multline*}
The last three terms are proportional to the energy flux $\vec{D}$ carried by diffusion of water. The first three terms can be rewritten using the definition of enthalpy and the total enthalpy gradient \eqref{e:enthalpy_gradient}, which leads to the residual SGS energy flux
...
Near the bottom boundary, Monin-Obukhov similarity theory is typically used for modeling the fluxes of momentum, heat, water vapor, and tracers in the constant-flux layer near the surface. This is adequate as long as the lowest model level is within the constant-flux layer (below about 50~m altitude). Care must be taken to formulate the surface fluxes accurately for finite-volume discretizations  \citep{Nishizawa18a}.

Advective fluxes normal to the surface vanish. Since the viscous sublayer is neither resolved in GCMs nor in LES, the bottom boundary condition is applied to the SGS fluxes of momentum, water, energy, and tracers. The bottom boundary condition is usually based on bulk exchange formulas, which linearly relate SGS fluxes to near-surface gradients normal to the surface.
...
\end{align}
\end{subequations}
Here, $\vec{n}$ is the upward pointing normal vector to the surface and $\vec{u} = (u, v, w)$ is the three-dimensional velocity vector. All quantities on the right-hand side are evaluated at a small height $z$ above the surface, usually in the lowest model level. This formulation for momentum exchange results from approximating the velocity gradient by a first-order finite difference and taking the velocity at the surface itself to vanish, $\vec{u}_\mathrm{sfc} =0$: this is the no-slip condition, which is not explicitly resolved in our model because the viscous sublayer is not resolved. The drag coefficient  $C_D(z)$ generally is a function of height $z$ and stratification of the atmosphere near the surface; it is usually determined from Monin-Obukhov similarity theory. At a flat surface, $\vec{n}$ is the vertical unit vector $\vec{k}$, and the vertical velocity $w(z)$ vanishes because the flow normal to the surface vanishes in the lowest model layer,
\[
\tau_{33, \mathrm{sfc}} = 0.
\]
...
\end{equation} 
where $C_q(z)$ is the exchange coefficient for water vapor. The surface specific humidity $q_{v, \mathrm{sfc}}$ is the specific humidity in a microscopic air layer in a contact with the surface. Over a wet surface, $q_{v, \mathrm{sfc}}$ is the saturation specific humidity at the surface temperature $T_\mathrm{sfc}$ (see section~\ref{sct:sat_spef_hum}): 
\[
...
In the SGS flux of total energy normal to the surface, $\vec{n} \cdot \rho (\vec{J} + \vec{D})$, the kinetic energy component in the lowest model level is usually ignored; it is 3--4 orders of magnitude smaller than the thermal and potential energy components. Therefore, consistent with the SGS energy flux \eqref{e:SGS_energy_flux}, the energy flux at the surface (units $\mathrm{W~m^{-2}}$) takes the form
...
\end{equation}
That is, the energy flux at the surface is a flux of moist static energy $h + \Phi$. It is proportional to the moist static energy  difference between the lowest model level and the surface. Over a wet surface, the enthalpy at the surface, $h_\mathrm{sfc}$, is evaluated with the saturation specific humidity $q_v^*(T_\mathrm{sfc})$ at the surface temperature $T_\mathrm{sfc}$. Consistent with the assumed vanishing of the condensate fluxes \eqref{e:sfc_condensate_flux}, the condensate specific humidities at the surface are set to their values at the lowest model level, so that their gradients normal to the surface vanish:
\[
...
To get a sense of how the ARK approach works, let us partition the total tendency $\vec{\mathcal{T}}$ on the right-hand side of Eq.~\eqref{e:eom_compact}, 
%\[
...
\label{eq:IMEX}
\ee
\hl{[Below you already seem to assume that $\Tvector_\mathrm{fastest}$ is linear, but this is not what we wrote in the abstract model formulation above. This needs to be clearer here: perhaps partition $\Tvector_\mathrm{fastest}$ into a linear component and a residual here? Or introduce linearization in the abstract model formulation above?]} To discretize the equations in time, we first compute the stage values
\begin{eqnarray}
...
\fxg{We still need to work out the Butcher tableau but, yes, there are stage values for all components except that for the slower modes some rows of $a$ are zero and so are skipped. But we need to write them together for mathematical rigor.} \hl{[Ok. It would help to make clear that some rows of $a$ will be zero.]} 
...
where $\Tvector_{\mathrm{fastest}}=-\nabla \cdot \Fvector_{\mathrm{fastest}} + \Source_{\mathrm{fastest}}$
 ...
 \]
 where we start with the fluxes given by Eqs.\ \eqref{e:adv_flux}--\eqref{eq:diff_flux} and write their linear representations as follows \hl{[I think you want to include here the non-diffusive fluxes (69) (radiation, falling precipitation) with $\rho_0$ in place of $\rho$]}
 \begin{equation}
...
\end{equation}
which are linear with respect to $\rho'$, $\rho \vec{u}$ (and $\vec{u}$ by extension  \hl{[TS: I don't understand this. It does not look linear in $\vec{u}$ because $\rho = \rho_0 + \rho'$, so that $\rho \vec{u}$ contains products of propagating perturbations.]}), and $(\rho \etotal - \rho_0 e_0)$.
\fxg{In principle, we can include the diffusive terms but there are vices and virtues to doing this. For one, we will not be able to extract a straightforward Schur complement (or at all).  Even without a Schur complement, we need to make the implicit solve dimension as small as possible. Using GCR or GMRES which require storing Arnoldi vectors for each iteration.  This will place a limit on the size of the problem that we can do on the GPU.  So the question is what do we gain from adding the diffusive fluxes.  Typically we are in the high Reynolds number regime so the stiffness will always be the acoustic waves.} \textcolor{red}{TS: The Reynolds number is irrelevant here (because it is so large, molecular viscosity/diffusion plays no role). The problem lies with SGS turbulent fluxes we need to model, and they can be large. However, let's leave the diffusive fluxes out of the implicit solve for now if that simplifies matters. They may not be crucial to treat implicitly (although most models do treat them implicitly, which helps at coarse resolution.}
...
\Source_{\mathrm{fastest}} = \left( \begin{array}{c}
 ...
 -2 \vec{\Omega} \times \rho\vec{u} - \rho' \nabla\Phi + \rho_0 \vec{F}_u \\
\rho_0 Q -\sum_{j\in\{v,l,i\}} (I_j + \Phi)  \rho_0 C(q_j \rightarrow q_p) + \rho_0 \vec{u} \cdot \vec{F}_u - M \\

\rho_0 C(q_p \rightarrow q_k) + \rho_0 \sum_j C(q_j \rightarrow q_k) \\
\rho_0 \sum_k C(q_k \rightarrow q_{p, i}) - \rho_0 \sum_j C(q_{p, i} \rightarrow q_{p, j})\\
\rho_0 \mathcal{S}_{\chi_i}
\end{array}
\right).
\label{eq:source_linear}
\ee
\hl{[You want $\rho_0$ in the Coriolis term, don't you? Also added the missing drag and heating terms.]}
\fxg{Need to look closer at the terms in the Source function to see what can be linearized.} \hl{[Some of the $C$ functions are linear in our current implementation, but they may not always be. Let's not worry about this now. It may be better if we allow the microphysics ($C$'s) to be treated explicitly with substepping.]}
...
\right),
\Fdiff_{\mathrm{fastest}}=\left( \begin{array}{c}
 \rho_0 \vec{d}_{q_t} \cdot \vec{k} \\
 \rho_0 \vec{\tau} \cdot \vec{k} \\
 \rho_0 \vec{J} + \rho_0 \vec{D} \cdot \vec{k}\\
\rho_0 \vec{d}_{q_k} \cdot \vec{k} \\
\rho_0 \vec{d}_{q_{p, i}} \cdot \vec{k}\\
\rho_0 \vec{d}_{\chi_j} \cdot \vec{k}
\end{array}
\right)
\label{eq:fluxes_linear_1d}
\end{equation}
which are linear with respect to $\rho'$, $\rho w$ (and $w$ by extension \hl{[TS: same as above, I don't understand this]}), $(\rho \etotal - \rho_0 e_0)$, and $p'$.  The term $\vec{k}$ in $\Fdiff_{\mathrm{fastest}}$ represents the unit vector along the $z$ direction.
...
\begin{equation}
 ...
 \diff{}{z}\left(  (\rho_0 e_0 + p_0) w \right) \\
 \vec{0}
\end{array}
\right)
...
With the $\Tvector_{\mathrm{fastest}}$ operator defined, we can then define the remaining nonlinear operator as $\Tvector - \Tvector_{\mathrm{fastest}}$ and follow the procedure outlined in Sec.\ \ref{sec:3D-IMEX/v1}. 
...
P &=& {\tt MoistThermodynamics.air_pressure(T - the reference temperature , \rho, q_t, q_l, q_i)}\\
