...
H(z)  = \frac{R_d T(z)}{g}
\end{equation}
is the scale height, and we chose for the pressure $p(0)$ at height $z=0$ the mean sea-level pressure $p_{\mathrm{MSLP}}$, a model parameter. (For the simple and commonly used choice of a constant initial temperature $T$, this reduces to $p(z) = \exp (-z/H)$, where the scale height $H = R_d T/g$ is constant.)

Given a temperature profile $T(z) = \max(T_s-\Gamma z, T_{\min})$, with surface temperature $T_s$, lapse rate $\Gamma$, and a minimum temperature $T_{min}$, integrating eq. \ref{eq:hydro_pressure} yields
\begin{equation}\label{eq:reftemperature}
p(z) = p_{\mathrm{MSLP}} \exp
\left[
-\frac{g}{R_d\Gamma} \left( \log(T_s) - \log(T_{\min}) + \Gamma z - T_s + T_{\min} \right)
\right], z>\frac{T_s - T_{\min}}{\Gamma}
\end{equation}


\hl{[If we are not well setup for the vertical integration, just implementing the case for constant temperature would suffice for now.]} The reference density then follows from the ideal gas law as
...
\end{equation}
where $q_v^*(T, \rho)$ is the saturation specific humidity \eqref{eq:sat_shum}. Computing the specific internal energy $I$ from these quantities using Eq.~\eqref{eq:total_internal_energy} (with $q_l = q_i = 0$) and with that the total specific energy $e^{\mathrm{tot}} = I + \Phi$ corresponding to state of rest ($\vec{u}=0$) then completes the specification of the model state.
...
\section{Spatial Discretization Methods}
...
\hl{Frank/Lucas/Jeremy: Can you write a little more about:}
\begin{itemize}
    \item \hl{Diffusion terms (second derivatives)}
    \item \hl{Boundary conditions (how are they implemented?)}
    \item \hl{How do we get higher-order derivatives (e.g., $\nabla^4$ for hyperdiffusion)? (It would be helpful for GCM, but clearly is a challenge with DG.)}
\end{itemize}
...
To circumvent the time-step restriction due to the fast acoustic waves, using implicit-explicit (IMEX) methods is one option. For the LES model, if the aspect ratio of the horizontal to vertical grid spacing is near unity, it may be beneficial to use fully 3D-IMEX methods.  For LES models with aspect ratios of grid elements greater than one, and certainly for global atmospheric models, which generally have large aspect ratios, we use 1D-IMEX methods in which the time-integrator is fully explicit in the horizontal direction (HE) and implicit in the vertical direction (so-called HEVI schemes). IMEX methods require a linear solve at each time step. This
is global in 3D. This may limit scalability of IMEX methods in a multi-node setting. In that case, split-explicit methods may be a more scalable option to deal with the timestep restrictions. For HEVI schemes, the linear solves are restricted to independent atmospheric columns, which are usually represented on a single node (if the domain decomposition is done in the horizontal). So their scalability across nodes is less restricted.
...
\]
Let us further partition the tendency $\Tvector_\mathrm{fastest}$ containing the fastest waves in the system into a component linear in state variables and a residual nonlinear component \hl{[if we distinguish the terms by integer labels, we should make clear where the nonlinear part of the fastest term goes, whose linear component is treated implicitly; default would be with the dynamical terms?]}
\[
\Tvector_\mathrm{fastest}(\statevec) =  \Tvector^L_\mathrm{fastest} \statevec + \Tvector^N_\mathrm{fastest}(\statevec).
\]
This then allows us to write the semi-discrete form (in space) as follows
\begin{equation}\label{eq:IMEX}
\difft{\statevec}{t} =  \Tvector^L_\mathrm{fastest} \statevec + \Tvector_\mathrm{fast}(\statevec) + 
\Tvector_\mathrm{dynamical}(\statevec) + \Tvector^N_\mathrm{fastest}(\statevec) + \Tvector_\mathrm{slow}(\statevec).
\end{equation}
To discretize the equations in time, we first compute the stage values
\begin{multline}\label{eq:IMEX/stages}
\statestage^{(i)}= \statevec^n + \Delta t \sum_{j=0}^{i} \left( a^{\mathrm{fastest}}_{ij} \Tvector_{\mathrm{fastest}}^L \statestage^{(j)} \right) + \Delta t \sum_{j=0}^{i-1} \left( {a}^{\mathrm{fast}}_{ij} \Tvector_{\mathrm{fast}}(\statestage^{(j)}) \right)  \\
+  \Delta t \sum_{j=0}^{i-1} \left( {a}^{\mathrm{dynamical}}_{ij} \bigl(\Tvector_{\mathrm{dynamical}}(\statestage^{(j)}) + \Tvector^N_\mathrm{fastest}(\statestage^{(j)})\bigr)\right) \\
+ \Delta t \sum_{j=0}^{i-1} \left( {a}^{\mathrm{slow}}_{ij} \Tvector_{\mathrm{slow}}(\statestage^{(j)}) \right)
\end{multline}
with $i=1,\ldots,s$ representing the $s$ stages, $\statestage^{(0)}=\statevec^n$ (where $\statevec^n$ is the solution vector at the current time), and $a$ are the coefficients of the partitioned Butcher tableau (see, e.g., \citet{constantinescu:2007, constantinescu:2010c}). Here, the nonlinear component of the fastest term was assumed to evolve with other terms on dynamical timescales. Some rows of the coefficient matrices $a$ for the slower component will be zero. \hl{We need to write this in a way that facilitates implementation, potentially with more subdivisions of a timestep. It makes sense to me to subcycle the faster components by an integer multiple of the dynamical timestep, and correspondingly to superstep the slow components by another integer multiple. Can we write this out explicitly now, even without having the Butcher tableaus worked out? We need this immediately once IMEX works and need to work on the implementation aspects now. We also need a split-explicit implementation as a fallback if 3D IMEX does not give us the performance increase we need.} 
The solution at time $n+1$ is obtained from
\be
\statevec^{n+1}=\statevec^n + \Delta t \sum_{i=0}^{s} \left( b_i \vec{\mathcal{T}}(\statestage^{(i)}) \right).
\label{eq:IMEX/update}
\ee
So far we have defined a diagonally-implicit Runge-Kutta (DIRK) method \citep{alexander:1977,butcher:1981a,ascher:1997,boscarino:2009}.  To make the DIRK more efficient, we impose the restriction that all diagonal values $\wt{a}_{ii}$ are constant \hl{[the same?]}. This allows one construction of the matrix problem that does not change across stage values.  This we now refer to as singly-diagonally-implicit Runge-Kutta (SDIRK).

Rearranging Eq.\ \eqref{eq:IMEX/stages} as follows \hl{[what happened to the off-diagonal elements involving $\Tvector^L_{\mathrm{fastest}}$? it seems DIRK assume $a_{jl} = 0$ for $j>l$, right?]}
\begin{multline}\label{eq:IMEX/stages2}
\left( \vec{I} - {a}^{\mathrm{fastest}}_{ii}\Tvector^L_{\mathrm{fastest}} \right)  \statestage^{(i)}  =  \statevec^n + 
\Delta t \sum_{j=0}^{i-1} \left( {a}^{\mathrm{fast}}_{ij} \Tvector_{\mathrm{fast}}(\statestage^{(j)}) \right)   \\
+ \Delta t \sum_{j=0}^{i-1} \left( {a}^{\mathrm{dynamical}}_{ij} \bigl(\Tvector_{\mathrm{dynamical}}(\statestage^{(j)}) + \Tvector^N_\mathrm{fastest}(\statestage^{(j)})\bigr) \right) \\
+ \Delta t \sum_{j=0}^{i-1} \left( {a}^{\mathrm{slow}}_{ij} \Tvector_{\mathrm{slow}}(\statestage^{(j)}) \right) 
\end{multline}
reveals the implicit nature of the problem. Letting $\vec{A}=\vec{I} - {a}^{\mathrm{fastest}}_{ii} \Tvector^L_{\mathrm{fastest}}$, $\vec{X}=\statestage^{(i)}$, and denoting the right-hand side of Eq.~\eqref{eq:IMEX/stages2} by $\mathcal{B}$, we obtain the linear system 
\[
...
Let us assume the fastest tendency terms $\Tvector_{\mathrm{fastest}}$ contain contributions from advection and pressure terms so that sound waves are captured:
 \begin{equation}
 \Tvector_{\mathrm{fastest}}=-\nabla \cdot \left( \Fadv_{\mathrm{fastest}} + \Pvector_{\mathrm{fastest}}\right).
 \label{eq:3d-imex/linear_operator}
 \end{equation}
...
\end{array}
\right)
\label{eq:3d-imex/linear_fluxes}
\end{equation}
\comment{
And the linear component of the source term  \hl{[TS: I would remove the gravity wave (buoyancy) terms. Gravity waves are not rate limiting in LES settings and are usually treated explicitly. That way you can also use the standard Euler Schur complements. Note that if the buoyancy term is omitted, this affects how the pressure gradient term is implemented: if the buoyancy term is omitted, the hydrostatic reference state contribution in the pressure terms needs to be kept, since the reference state part included in the total (nonlinear) buoyancy term also remains.]}
is 
\begin{equation}
...
This is linear with respect to $\rho'$, $\rho \vec{u}$, and $p'$, which is linear in state variables provided we express the perturbation  pressure $p'= p - p_r$ as a linear function of state variables. We can do this by first computing the linearized internal energy perturbation per unit volume,
\begin{equation}
(\rho I)' \approx (\rho \etotal)'  -  \rho' \Phi,
\end{equation}
assuming a reference state at rest, and computing from that $(\rho T)'$ using a version of the temperature equation~\eqref{eq:temperature} that is linear in the state variables:
\begin{equation}\label{eq:temperature_perturb}
(\rho T)' =  \frac{(\rho I)'  - (\rho q_t - \rho q_l)I_{v,0} + (\rho q_i) (I_{i,0} + I_{v,0})}{c_{vd}}.
\end{equation}
The ideal gas law (neglecting moisture perturbations affecting the gas constant) then gives the perturbation pressure \hl{[It would be good to add a function to MoistThermodynamics that returns this linearized pressure perturbation. We'll need it in all IMEX variants. \textbf{Charlie?}]}
...
Defining 
\[
\Tvector^L_{\mathrm{fastest}} \statevec  = - \nabla \cdot \left( \mathcal{L} \bigl(\Fadv_{\mathrm{fastest}}\bigr) + \mathcal{L}\left(\Pvector_{\mathrm{fastest}}\right) \right) ,
\]
we obtain the nonlinear component of the fastest tendency as the residual 
\begin{equation}\label{e:nonlinear_residual}
\Tvector^N_{\mathrm{fastest}}(\statevec) = \Tvector_{\mathrm{fastest}}(\statevec) - \Tvector^L_{\mathrm{fastest}} \statevec.
\end{equation}
...
\begin{equation}
 \vec{\mathcal{T}}(\statevec)=- \left( \begin{array}{c}
 0 \\
...
\subsubsection{3D-IMEX Linear Operator in the Reference Coordinate}
 ...
 $\Tvector_{\mathrm{fastest}}$ as follows:
 \begin{equation}
...
For grid aspect ratios $\Delta_h/\Delta_v \gg 1$, the stiffness of the equations arises principally from vertically propagating sound and gravity waves; vertical diffusion terms may also add stiffness. For this situation, we only need to extract sound and gravity waves, plus possibly diffusion terms, along the vertical direction and treat them implicitly.  Following an approach similar to that for the 3D-IMEX method given in Sec.\ \ref{sec:3D-IMEX/v1}, we write the fastest tendency term, involving only vertical flux divergences, as
 \[
 ...
 \]
 where $z$ again denotes the vertical coordinate.  Let us start with the fluxes given by Eqs.\ \eqref{e:adv_flux}--\eqref{eq:diff_flux} and write their linear components as follows: \hl{[the vector components do not match up here. First, it's odd to have the vertical coordinate in the first position of the velocity vector, third is common. Second, the entries for the momentum fluxes are to be understood as vector valued but are not indicated as such. Third, the diffusion terms do not match]}
 \begin{equation}
\mathcal{L}\left(\Fadv_{\mathrm{fastest}}\right)
=\left( \begin{array}{c}
 \rho w \\
 0 \\
 0 \\
...
\end{array}
\right), \qquad
\mathcal{L}\left(\Pvector_{\mathrm{fastest}}\right)
= \left( \begin{array}{c}
0 \\
0 \\
0 \\
p' \\
\frac{p_r}{\rho_r} (\rho w) \\
0 \\
0 \\
0 
\end{array}
...
and
\begin{equation}
\mathcal{L}\left(\Fdiff_{\mathrm{fastest}}\right)=\left( \begin{array}{c}
...
\begin{equation}
 ...
 \label{eq:1d-imex/linear_operator}
 \end{equation}
...
\end{array}
\right),
\label{eq:1d-imex/linear_fluxes}
\end{equation}
...
\end{array}
\right).
\label{eq:1d-imex/linear_sources}
\end{equation}
All of these are linear with respect to $\rho'$, $\rho w$, and the linearized pressure $p'$ obtained from Eq.~\eqref{eq:pressure_perturb}. The term $\rho w$ represents the momentum along the vertical ($z$) direction. The source term includes the geopotential responsible for gravity waves. It makes sense to include them in the implicit part in 1D IMEX, because we usually are not interested in an accurate representation of gravity waves in a global model.

With the linear operator
\[
\Tvector^L_{\mathrm{fastest}} \statevec  = - \diff{}{z} \left( \mathcal{L}\left(\Fadv_{\mathrm{fastest}}\right) + \mathcal{L}\bigl(\Pvector_{\mathrm{fastest}}\bigr)
+\mathcal{L}\bigl(\Fdiff_{\mathrm{fastest}}\bigr)\right) + \mathcal{L}\bigl( \Source_{\mathrm{fastest}}\bigr)
\]
defined, we can then define the remaining nonlinear operator as the residual \eqref{e:nonlinear_residual} and otherwise follow the procedure outlined in section~\ref{sec:3D-IMEX/v1}. 
