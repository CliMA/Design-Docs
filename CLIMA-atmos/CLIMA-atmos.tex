...
\end{equation}
[\fxg{What happened to $\Tvector^N_{I}(\vec{Y})$? I've added it back in.  We can use this form whereby the total variable is used instead of the perturbation but if we use an iterative solver then it behooves us to use warm restarts, otherwise the iterative solver has to work harder to get the solution to get to the reference state and then to the perturbation from the reference state. This situation is easy to see for cases when the solution is not far away from the reference field.  If not the case, then is not too big of an issue.}] \hl{[Thanks for adding the missing term. And, of course, you start the iterations from the reference state, which is the same as starting from zero when working in perturbed variables. Might it not be even better to start from the state at a previous time step?}
...
[\fxg{I will assume below that $\Tvector_I(\vec{Y}_r) - \vec{\mathcal{L}}_I \vec{Y}_r=0$.}]

Assuming that $\Tvector_I(\vec{Y}_r) - \vec{\mathcal{L}}_I \vec{Y}_r=0$, let us
describe the 3D-IMEX approach, by focusing on the simpler form of Eq.\ \eqref{eq:IMEX} written as follows
\begin{equation}
\label{eq:IMEX_v2}
\frac{\partial\vec{Y}}{\partial t} =  \vec{\mathcal{L}}_I \vec{Y} + \Tvector^N_{I} + \Tvector_{d} + \Tvector_{-s}.
\end{equation}
which we shall rewrite in the following way
\begin{equation}
\label{eq:IMEX_v3}
\frac{\partial\vec{Y}}{\partial t} =  \vec{\mathcal{L}}_I \vec{Y} + \Tvector_{p} + \Tvector_{-s}
\end{equation}
where $\Tvector_{p}=\Tvector^N_{I} + \Tvector_{d}$ and the subscript $p$ denotes the "physical" mode, for lack of a better term (is what Wicker and Skamarock call this mode - \textbf{need reference}).
\begin{algorithm}
...
\end{equation}
where $A=a_{ij}, \; i,j=1,\ldots S$, and $c_i=\sum_{j} a_{ij}$ represent the time when the right-hand side is evaluated; that is, we evaluate it at each stage which occurs at the time interval $t+c_i \dt$.
...
\EndFor %i
\State $\vec{Y}^{n+1}=\vec{Y}^{n} + \Delta t \sum_{j=1}^{i} b_i \left[ \Tvector_{-s}(\red{\vec{Y}^{n-s}}) + \Tvector_{p}(\vec{Y}_i^{(i)}) + 
...
[\fxg{Would be good to know if we plan to use such states. If not then the 1D-IMEX problem becomes very simple for us and so we can optimize based on this since only one linear system has to be solved for all columns but with different RHS.}] \hl{[Let's focus on hydrostatic reference states at rest and implement that. If the implicit solve only contains sound waves, that will be fine for sure, and it is probably also good enough for gravity waves. And, yes, that means it is the same system for each column; so we should be able to store an LU or QR factorization and solve directly.]}
...
Let us lay out in general terms the linearizations for acoustic and gravity waves that underlie IMEX methods in 1D and 3D. The tendency terms responsible for acoustic waves and gravity waves are
 ...
 \rho \vec{u} \\
 p \vec{I}_3 \\
 \bigl(e^{\mathrm{tot}} + (\delta_{gw}-1) \Phi + p/\rho \bigr) \rho \vec{u} \\
 \vdots
 \end{pmatrix}
 - \begin{pmatrix}
 0 \\
...
In the tendency \eqref{eq:3d-imex/tendencies}, the terms that need to be linearized are the pressure $p$, which depends nonlinearly on state variables, and the total enthalpy flux $(\rho e^{\mathrm{tot}} + p/\rho) (\rho \vec{u})$. Linearization around a state of rest ($\vec{u}_r=0$) with reference total energy $e^{\mathrm{tot}}_r$, pressure $p_r$, and density $\rho_r$ leads to
 ...
 \Bigl(e^{\mathrm{tot}}_r  + (\delta_{gw}-1)\Phi + p_r/\rho_r \Bigr) \textcolor{blue}{\rho \vec{u}}\\
\vdots
\end{pmatrix}
-
\begin{pmatrix}
0 \\
\delta_{gw} \textcolor{blue} \rho \nabla_{1/3} \Phi \\
0\\
\vdots
\end{pmatrix}.
\end{equation}
...
\begin{itemize}
    \item Acoustic and gravity waves included ($\delta_{gw}=1$). In this case, it is evident that $\Tvector_I(\vec{Y}_r)=0$ because the pressure gradient ($-\nabla_{1/3} p_r(z)$) and geopotential gradient ($-\rho_r\nabla_{1/3}\Phi$) in the momentum equation (second component of tendency vector) cancel for a hydrostatic reference state with $p_r = p_r(z)$. The same is true for the linear term for a reference state at rest, for which $p_L = p_r$: $\vec{\mathcal{L}}_I \vec{Y}_r = 0$. [\fxg{Is your intent to only use hydrostatic reference profiles?}] \hl{yes}
    \item Only acoustic waves included ($\delta_{gw}=0$). In this case, neither $\Tvector_I(\vec{Y}_r)$ nor $\vec{\mathcal{L}}_I\vec{Y}_r$ vanish individually, because the geopotential term needed for the cancellation of terms in hydrostatic balance does not appear in the momentum tendency. However, $p_L = p_r$ in a reference state at rest, so that $\Tvector_I(\vec{Y}_r) = \vec{\mathcal{L}}_I\vec{Y}_r$.
\end{itemize}
...
For grid aspect ratios $\Delta_h/\Delta_v \gg 1$, the stiffness of the equations arises principally from vertically propagating sound and gravity waves; vertical diffusion terms may also add stiffness. For this situation, we only need to extract fast terms along the vertical direction and treat them implicitly.  Following the general approach outlined in section~\ref{s:IMEX_general}, using $\delta_{gw}=1$ to extract sound and gravity waves. (We treat diffusion terms explicitly for the moment.) The linear tendency component $\vec{\mathcal{L}}_I \vec{Y}$ and the nonlinear residual $\Tvector_I^N(\vec{Y})$ are then given by Eqs.~\eqref{e:IMEX_linear} and \eqref{e:nonlinear_residual}, with the derivative operator $\nabla_{1/3} = \vec{k} \partial/\partial z$.
...
To discretize the equations in time using a partitioned Runge-Kutta method, let us describe the method using the following 4-partition system of ordinary differential equations:
\[
\diff{\vec{Y}}{t}= L_I(\vec{Y}) + \Tvector_{+f} + \Tvector_{p} + \Tvector_{-s}
\]
where $\Tvector_{p}=\Tvector^N_{I}(\vec{Y}) + \Tvector_{d}$, and the right-hand side is ordered in ascending wave-speed order, i.e., 
the wave speed $\mathcal{S}$ of the different components are ordered as follows $\mathcal{S}(L_I) > \mathcal{S}(\Tvector_{+f})$, etc.

The algorithm describing the 4-partition multirate RK method is highlighted in Alg.\ \ref{alg:4-prk}.
\begin{algorithm}
\label{alg:4-prk}
\begin{algorithmic}
\State
...
\end{algorithmic}
\end{algorithm}
In Alg.\ \ref{alg:4-prk} the red, blue, and magenta fonts indicate terms that are frozen within the $k$-loop and is what allows a performance gain since these terms are not computed within this loop. Also, 
$\Delta \tau=\Delta t a_i^I a_j^J a_k^K$ where $\Delta t$ is the time-step of the physical mode, the RK coefficients are written in low-storage form (e.g., $a_i^I=\frac{1}{I-i+1}$ yields one common method for $I\leq3$), and the order of each partition can be of different order. If all partitions are of the same order then the effective time-step of each partition is 
\[
