...
\begin{table}[h]
  ...
  \begin{tabular}{cl}
    ...
    $\diffusive^{*}$                               & \texttt{diffusive\_numerical\_flux!}\\
  \end{tabular}
\end{table}

\hl{Still TODO\@:}
\begin{itemize}
  \item Discuss quadrature/variational crimes?
  \item Discuss boundary conditions
  \item State choices for fluxes based on \citet{bassi:1997nse}
  \item Bring code in line with skew symmetric formulation
  \item Replace in code \texttt{diffusive\_penalty!} with
    \texttt{diffusive\_numerical\_flux!}
