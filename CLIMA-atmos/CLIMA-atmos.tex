...
\begin{figure}
    \centering
    \includegraphics{CLIMA-atmos/hydrostatic_state.pdf}
...
Other non-diffusive fluxes include source terms (e.g., radiation) that can be written in flux form but that are not proportional to gradients of state variables, as well as the fall velocity $w_p$ (approximately terminal velocity) of falling precipitate:
\begin{equation}
\Fndf = 
\left( \begin{array}{c}
\vec{0} \\
...
\begin{itemize}
    \item Acoustic waves. They have phase speeds (Eq.~\eqref{e:soundspeed}) around $300~\mathrm{m~s^{-1}}$. The terms involving pressure in the momentum and energy equations are essential for their existence.
...
We use a general family of additive Runge-Kutta methods (ARK) methods for 1D and 3D IMEX approaches (see, e.g., \citet{giraldo:2013}). 
...
This gives
\[
\frac{\partial \vec{Y}}{\partial t} = \Tvector = - \nabla \cdot \Fvector + \vec{\mathcal{S}} = \Tvector_{I} + \Tvector_{d} + \Tvector_{+r} + \Tvector_{-s}.
\]
...
\frac{\partial\vec{Y}}{\partial t} =  \vec{\mathcal{L}}_I \vec{Y} + \Bigl(\Tvector_I(\vec{Y}_r) - \vec{\mathcal{L}}_I \vec{Y}_r\Bigr) + \Tvector_{d} + \Tvector_{+r} + \Tvector_{-s}.
\end{equation}
Usually, the reference state $\vec{Y}_r$ is chosen so that the tendency of the reference state, $\Tvector_I(\vec{Y}_r)$, and the linear tendency of the reference state, $\vec{\mathcal{L}}_I \vec{Y}_r$, either vanish individually or cancel each other, so that $\Tvector_I(\vec{Y}_r) - \vec{\mathcal{L}}_I \vec{Y}_r=0$

To discretize the equations in time using an ARK method, we first compute the stage values \hl{\textbf{Frank}: This subsection below here needs work. A number of things are unclear/incorrect/}
...
More sophisticated reference states for linearization are possible, for example, with a latitudinally varying hydrostatically balanced temperature profile, and with a reference velocity in geostrophic and hydrostatic balance with this temperature field. 
...
Let us lay out in general terms the linearizations for acoustic and gravity waves that underlie IMEX methods in 1D and 3D. The tendency terms responsible for acoustic waves and gravity waves are
 ...
 \rho \vec{u} \\
 p \vec{I}_3 \\
 \bigl(e^{\mathrm{tot}} + (\gamma-1) \Phi + p/\rho \bigr) \rho \vec{u} \\
 \vdots
 \end{pmatrix}
 - \begin{pmatrix}
 0 \\
 ...
 \end{equation}
where all components indicated by dots are zero. The operator $\nabla_{1/3}$ is the 3D del operator $\nabla_3 = \nabla$ for 3D-IMEX, and it is the 1D vertical operator $\nabla_1 = \vec{k} \partial/\partial_z$ for 1D-IMEX. (The geopotential gradient $\nabla_{1/3} \Phi$ only has a vertical component as long as we consider a spherical planet, so this derivative even in 3D usually only has a vertical component.) The function $\gamma$ is included to indicate whether gravity waves are included in the implicit tendencies: 
\begin{itemize}
    \item $\gamma=1$: both acoustic and gravity waves are included in $\Tvector_I$;
    \item $\gamma=0$: only acoustic waves are included in $\Tvector_I$.
\end{itemize}

In the tendency \eqref{eq:3d-imex/tendencies}, the terms that need to be linearized are the pressure $p$, which depends nonlinearly on state variables, and the total enthalpy flux $(\rho e^{\mathrm{tot}} + p) \vec{u}$. Linearization around a state of rest ($\vec{u}_r=0$) with reference total energy $e^{\mathrm{tot}}_r$, pressure $p_r$, and density $\rho_r$ leads to
 \begin{equation}
 \vec{\mathcal{L}}\vec{Y} = 
 -\nabla_{1/3} \cdot \begin{pmatrix}
 \rho \vec{u} \\
 p_L \vec{I}_3  \\
 \Bigl(e^{\mathrm{tot}}_r  + (\gamma-1)\Phi + p_r/\rho_r \Bigr) \rho \vec{u}\\
\vdots
\end{pmatrix}
-
\begin{pmatrix}
...
\end{equation}
This is linear with respect to $\rho$ and $\rho \vec{u}$. For it to be a linear function of all state variables, the pressure $p_L$ needs to be expressed as a linear function of state variables. To derive a linear approximation for the pressure, we use the ideal gas law $p = R_m (\rho T)$ and the expression derived from Eq.~\eqref{eq:temperature} for $\rho T$ in terms of the internal energy and specific humidities,
\begin{align*}
p &= R_m (\rho T) \\
...
\end{align*}
where we have used the relation $I = e^{\mathrm{tot}} - \Phi - 0.5 \|\vec{u}\|^2$ between total and internal energy. Linearization $p_L = p_r + (\partial p/\partial\vec{Y})(\vec{Y}-\vec{Y}_r)$ around the reference state $\vec{Y}_r$ with pressure $p_r$ and zero specific humidities leads to 
\begin{equation}\label{eq:pressure_linear}
p_L = \rho R_d T_0 + \frac{R_d}{c_{vd}} \bigl[ \rho e^{\mathrm{tot}} - \rho \Phi - (\rho q_t - \rho q_l)I_{v,0} + (\rho q_i) (I_{i,0} + I_{v,0}) \bigr].
\end{equation}
Note that the temperature $T_0$ that appears here is the \emph{reference temperature in the definition of internal energy \eqref{e:internal_energies}}, a model constant; it is \emph{not} the temperature $T_r$ of the reference state about which we linearized. In fact, the linearized pressure \eqref{eq:pressure_linear} does not depend on the reference state: the zeroth-order term $p_r$ in the linearization cancels the terms involving the reference state in the linear term, which ensures that the linearized pressure vanishes, e.g., when the density vanishes. 
\hl{[It would be good to add a function to MoistThermodynamics that returns this linearized pressure. We'll need it in all IMEX variants. \textbf{Charlie?}]}

We can now see that the term $\Tvector_I(\vec{Y}_r) - \vec{\mathcal{L}}\vec{Y}_r = 0$ involving the reference state in the decomposition \eqref{e:imex_linearization} vanishes. It is useful to distinguish the cases when gravity waves are included in the implicit term and when they are not included: \hl{[check this]}
\begin{itemize}
    \item Acoustic and gravity waves included ($\gamma=1$). In this case, it is evident that $\Tvector_I(\vec{Y}_r)=0$ because the pressure gradient ($-\nabla_{1/3} p_r(z)$) and geopotential gradient ($-\rho_r\nabla_{1/3}\Phi$) in the momentum equation (second component of tendency vector) cancel for a hydrostatic reference state with $p_r = p_r(z)$. The same is true for the linear term for a reference state at rest, for which $p_L = p_r$: $\vec{\mathcal{L}}\vec{Y}_r = 0$.
    \item Only acoustic waves included ($\gamma=0$). In this case, neither $\Tvector_I(\vec{Y}_r)$ nor $\vec{\mathcal{L}}\vec{Y}_r$ vanish individually, because the geopotential term needed for the cancellation of terms in hydrostatic balance does not appear in the tendency term. However, $p_L = p_r$ in a reference state at rest, so that $\Tvector_I(\vec{Y}_r) = \vec{\mathcal{L}}\vec{Y}_r$.
\end{itemize}
...
\begin{split}
\Tvector^N_{I}(\vec{Y}) =  \Tvector_I(\vec{Y}) - \vec{\mathcal{L}} \vec{Y}.
\end{split}
\end{equation}
...
For grid aspect ratios $\Delta_h/\Delta_v \gg 1$, the stiffness of the equations arises principally from vertically propagating sound and gravity waves; vertical diffusion terms may also add stiffness. For this situation, we only need to extract sound and gravity waves, plus possibly diffusion terms, along the vertical direction and treat them implicitly.  Following an approach similar to that for the 3D-IMEX method given in Sec.\ \ref{sec:3D-IMEX/v1}, we write the fastest tendency term, involving only vertical flux divergences, as
 \[
 \Tvector_{I}=- \diff{}{z} \left( \Fadv_{I} + \Pvector_{I} + \Fdiff_{I} \right) + \Source_{I}
 \]
...
\end{array}
\right), \qquad
\mathcal{L}\left(\Pvector_{I}\right)
= \left( \begin{array}{c}
0 \\
0 \\
0 \\
...
and
\begin{equation}
\mathcal{L}\left(\Fdiff_{I}\right)=\left( \begin{array}{c}
...
\begin{equation}
 \Tvector_{I}=- \diff{}{z} \left( \Fadv_{I} + \Pvector_{I} \right) + \Source_{I}
 ...
 \begin{equation}
 \mathcal{L} \left( \Fadv_{I} \right)=\left( \begin{array}{c}
 \left( \rho w \right) \\
 0 \\
 0 \\
 0 \\
...
\end{array}
\right), \qquad
\mathcal{L} \left( \Pvector_{I} \right) = \left( \begin{array}{c}
0 \\
0 \\
0 \\
p' \\
...
\begin{equation}
\mathcal{L} \left( \Source_{I} \right) = \left( \begin{array}{c}
0 \\
0 \\
0 \\
-\rho' \diff{\Phi}{z} \\
...
\[
\Tvector^L_{I} \vec{Y}  = - \diff{}{z} \left( \mathcal{L}\left(\Fadv_{I}\right) + \mathcal{L}\bigl(\Pvector_{I}\bigr)
+\mathcal{L}\bigl(\Fdiff_{I}\bigr)\right) + \mathcal{L}\bigl( \Source_{I}\bigr)
\]
